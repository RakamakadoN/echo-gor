        ...
        <main className={`flex-1 flex flex-col p-4 md:p-6 space-y-6 overflow-y-auto min-w-0 ${activeRole === 'teacher' || activeRole === 'branch' || activeRole === 'owner' || activeRole === 'admin' ? 'p-0 md:p-0 space-y-0' : ''}`}>
          ...
          {activeRole === "chart" ? (
            ...
            <>
              ...
              {/* Легаси-дашборд владельца/управляющего/админа вытеснен ролевыми
                  воркспейсами и до этой else-ветки для них не доходит (тернарник
                  выше уводит их в OwnerExecutiveWorkspace/AdminEduErpWorkspace/…).
                  Обёртка не даёт старым вкладкам (overview/billing/logs…) случайно
                  отрендериться УЧЕНИКУ, у которого activeTab по умолчанию "overview". */}
              {(activeRole === "owner" || activeRole === "branch" || activeRole === "admin") && (<>
