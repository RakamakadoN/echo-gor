...
**Автоматически (билетные агрегаторы) — ТРЕБУЕТСЯ scraping-прокси:**

- **Ticketon** (`ticketon.kz`) — Казахстан + СНГ. Раздел `/{city}/concerts`, список — `TICKETON_CITIES`.
- **Kassir** (`kassir.ru`) — Россия (Москва, СПб). Список — `KASSIR_CITIES`.

⚠️ **Важно (проверено на проде 2026-06-20):** оба агрегатора закрыты бот-защитой.
Ticketon за Cloudflare режет серверные IP (Vercel → 403), Kassir отдаёт JS-заглушку.
Прямой серверный `fetch` не работает. Поэтому сетевой слой обёрнут в `proxiedFetch`,
который при заданном `SCRAPER_API_KEY` гоняет запросы через scraping-сервис
(ScraperAPI/ScrapingBee или любой по шаблону `SCRAPER_API_URL`). Переменные — см. `.env.example`.

- Ticketon — SSR, данные в HTML: хватит обычного прокси.
- Kassir — клиентский рендер: нужен `SCRAPER_RENDER=true` (дороже по кредитам).
- Без ключа парсер отработает «вхолостую» (0 событий из агрегаторов); ручной ввод работает всегда.

Диагностику по источникам см. в ответе `parse` — поле `debug` (`links`/`prefiltered`/`fetchErrors`).
