...
export function registerGeminiApi(app: express.Express) {
  ...
  app.post("/api/gemini/period-report", async (req, res) => {
    ...
    const scope = String(scopeLabel || (metrics && (metrics as any).область) || "Вся сеть");
    const isNetwork = scope === "Вся сеть";
    const audience = isMonth
      ? "месячная планёрка с управляющими филиалов: разбор БДР план/факт, чистая прибыль, удержание/отток, воронка, и план на текущий месяц"
      : "недельная планёрка с управляющими: итоги прошедшей недели и фокус-задачи на текущую неделю";
    const scopeRule = isNetwork
      ? "Область отчёта — вся сеть: где уместно, дай разбивку по филиалам."
      : `Область отчёта — ${scope}. НЕ давай разбивку по филиалам и по сети — сфокусируйся ТОЛЬКО на этой области. Задачи адресуй ответственному за эту область.`;
    const prompt = `Ты — операционный директор сети школ кавказского танца «Эхо Гор» (Казахстан). Составь деловой отчёт для планёрки владельца с управляющими. Тип: ${audience}. ${scopeRule} Верни СТРОГО JSON по схеме:
{"title": string, "tldr": string, "sections": [{"title": string, "points": string[]}], "focus": string[]}
Правила: пиши по-русски, кратко и по делу, как для совещания. Опирайся ТОЛЬКО на переданные цифры, не выдумывай. В title отрази область отчёта («${scope}»). tldr — 2-3 предложения главного (выполнение плана, прибыль, тренд). sections — 3-5 блоков по темам (Финансы и БДР, Продажи и воронка, Удержание и отток, Заполняемость и набор${isNetwork ? ", Филиалы" : ""}). В points — конкретика с числами и сравнением (мес/период назад), отмечай что просело/выросло. focus — 3-5 конкретных задач-приоритетов на ${isMonth ? "текущий месяц" : "текущую неделю"}. Если по какому-то показателю данных нет — не упоминай его.
